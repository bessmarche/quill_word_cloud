...
# Use NLTK Named Entity Recognition to prioritise Named Entities fin the word cloud, (by increasing their detected frequency by a factor of self.priority_NE).

def prioritize_NE(frequencies, factor):
    """

    :param frequencies:  dict from string to float
            A contains words and associated frequency.

           factor: the factor by wich the NE frequency is increased
    :return: a dict from string to float where the frequency of each NE is multiplied by factor
    """
    for key, value in frequencies.items():
        if is_name(key):
            frequencies[key] = value * factor
    return frequencies

def is_name(x):
    """

    :param x: a word, can be a bigram i.e. (United States)
    :return:
    """
    ...
    for child in ne_tree:
        if isinstance(child, tuple):
             name = name or False
        elif (child.label()) in ["PERSON", "GPE","ORGANISATION"]:
             name = name or True
    return name
...
class WordCloud(object):
    ...
    priority_NE: int, default=1
        The factor by wich all Named Entities frequencies are increased.
        The higher this factor is the more priority is given to NE in the wordcloud.
        .. added for Quill_Project
    ...
    def __init__(self, font_path=None, width=400, height=200, margin=2,
                 ...
                 include_numbers=False, min_word_length=0, priority_NE = 1):
    ...
    def generate_from_frequencies(self, frequencies, max_font_size=None):  # noqa: C901
        ...
        """
        # give priority to NE according to the parameter self.priority_NE
        if self.priority_NE > 1:
            frequencies = prioritize_NE(frequencies, self.priority_NE)
    ...
    def to_html(self):
            """Export to HTML, the wordcloud is represented as a SVG string.
            ...
            Consistent rendering is expected if both Pillow and the SVG
            ...
            -------
            content : string
                Word cloud image as HTML string
            """
            html_start = """
            <!DOCTYPE html> 
            <html>
                 <body>
                    <div id="canvas-word-cloud" style="background_colour:%s">    
                            %s    
                    </div>
